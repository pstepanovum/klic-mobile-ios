...
func unpackWaveform(_ data: Data) -> [Float] {
    ...
    for i in 0..<sampleCount {
        ...
        let value = (raw >> UInt32(bitOffset)) & 0x1F
...
func packWaveform(_ amplitudes: [Float]) -> Data {
    ...
    var bitCount = 0
    var bytes = [UInt8]()
    bytes.reserveCapacity((amplitudes.count * 5 + 7) / 8)
    for amp in amplitudes {
        let value = UInt32(min(31, Int(max(0, amp) * 31 + 0.5)))
    ...
    if bitCount > 0 { bytes.append(UInt8(bitBuffer & 0xFF)) }
    return Data(bytes)
}
